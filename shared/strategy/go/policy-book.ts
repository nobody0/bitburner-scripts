...
import { secretPolicyMove, secretPolicySize } from "./secret-book.ts";

export type GoPolicyBookOpponent = GoRewardOpponent;
...
export function goPolicyMove(
  ...
  if (opponent === "????????????") return secretPolicyMove(rows);
...
export function goPolicyEntryCount(opponent: GoPolicyBookOpponent): number {
  if (opponent === "Illuminati") return illuminatiPolicySize;
  if (opponent === "????????????") return secretPolicySize;
  return POLICY[opponent].size;
}
