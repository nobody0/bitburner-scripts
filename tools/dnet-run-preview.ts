...
const result = await runGame({
  ...
  onRecord: (line) => {
    ...
    if (typeof record.key === "string" && record.key.startsWith("dnet.overseer")) {
...
console.log(`overseer: ${JSON.stringify(latest.knowledge?.overseer)}`);
...
console.log(`controller: ${controllerSays.slice(0, 900)}`);
console.log(`-> ${out}`);
