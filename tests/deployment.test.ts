...
describe("stable deployment artifacts", () => {
  test("emits only the configured stable artifacts", async () => {
  ...
  test("owns directories the build writes into, and never the root", () => {
    ...
    // start.js and main.js sit at the root, which is shared with the game
    // and the player. A root-only config therefore owns nothing.
    expect(ownedDirectories(["start.js", "main.js"])).toEqual(new Set());
  });
