...
import { SimArtifactSession, type SimSessionManifest } from "../sim/artifacts.ts";
...
describe("hierarchical run identity", () => {
  ...
  test("a simulator session rotates JSONL artifacts after prestige", async () => {
    ...
    try {
      const session = new SimArtifactSession({ outDir: dir, label: "chain", seed: 7, bitNode: 1, createdAt: 1_000 });
      ...
      session.write(line(1, 60_000, "sim.prestige"));
      session.write(line(2, 65_000, "start.boot"));
      session.write(line(3, 181_000, "sim.result"));
      await session.close();
