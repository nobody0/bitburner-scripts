...
describe("tab rendering", () => {
  ...
  test("every feature's populated panel renders", () => {
    ...
    const topics: { [K in StateKey]?: StateMap[K] } = {
      ...
      factions: { joined: ["CyberSec"], standings: [{ name: "CyberSec", rep: 100, favor: 1, favorToDonate: 150 }], invites: ["NiteSec"], inviteRequirements: { NiteSec: ["hacking 200"] }, ownedAugs: ["BitWire"], offers: [{ name: "Rootkit", faction: "CyberSec", price: 1e6, repReq: 100, affordableRep: true, owned: false }], augTotal: 1, graftable: [{ name: "Rootkit", price: 1e6, timeMs: 6e4 }] },
