...
- Controller-facing models currently cover hacking, factions, crimes,
  ...
  gang, corporation, Bladeburner and sleeves remain unmodeled. The Darknet is FULL
  for fresh and multi-install controller runs: all 22 APIs, passwords and
  sessions, mutation/restart, labyrinth/storms, clue/cache rewards, live stock
  grants, coding-contract generation/solve/reward, and — since 2026-08-31 —
  hostnames generated by the transcribed `generateDarknetServerName` rather
  than a counter. `sim/tests/dnet-parity.test.ts` is the source-text suite
  `spec/game-source.md` requires before that word may be used at all.
