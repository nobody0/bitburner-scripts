...
from train_v9 import (ProposalExample, DistillExample, encode_states, load_v9,
...
def run(args: argparse.Namespace) -> None:
    ...
    records: list[tuple[ProposalExample, list[DistillExample]]] = []
    with gzip.open(args.corpus, "rt") as source:
        for line in source:
            ...
            if len(records) >= args.positions:
                break
    reply_values = [value for _, values in records for value in values]
    ...
    print(json.dumps({
        ...
        "weightedReplyWinMae": reply_win_error / max(reply_weight, 1e-9),
    }, indent=2))
