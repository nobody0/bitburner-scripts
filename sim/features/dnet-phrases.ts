/** `packetSniffPhrases` — the filler prose `capturePackets` and the log-noise
 * generator both draw from.
 *
 * Transcribed rather than paraphrased because the junk is the whole difficulty
 * of `OpenWebAccessPoint` above difficulty 16: the blob embeds the BARE
 * password in prose that itself contains digits, dictionary words and roman
 * numerals, so a solver that intersects candidate substrings across blobs has
 * to face the real distractors. Invented filler would be uniformly wrong in
 * whichever direction we happened to invent it.
 * Source: ../bitburner-src @ 3162fd2590e221eadd0c0fbd46151913f7c4c41c
 *   src/DarkNet/models/dictionaryData.ts:287-388 */
export const PACKET_SNIFF_PHRASES: readonly string[] = [
