...
import { captureExecLaunch, type ExecLaunchEntity } from "../lib/launch-shared.ts";
...
export async function main(ns: NS): Promise<void> {
  ...
  const entity = captureExecLaunch<DnetAgentLaunch>(ns, "dnet-agent");
  if (!entity) return;
  ...
  if (launch?.bootstrapReclaim === true) {
    const finish = (): void => live()?.bootstrapDone(host, ns.pid);
    ...
    try {
      const g = live();
      if (!g) return;
      g.registerBootstrap(host, entity);
  ...
  const pending = launch.order;
  if (g === undefined || entry === undefined || pending === undefined) return;

  await runAsOrder(ns, g, entry, pending, entity);
}
...
async function runAsOrder(
  ...
  // The death path settles as died and checks out with the original pid. The
  // controller drops the handle and proves the allocation retired.
  //
  ...
  // DELIBERATE EXIT. The generic launch atExit resolves this exact entity; its
  // controller subscriber keeps the host retiring until kill(pid) resolves,
  // proving the worker and its RAM allocation are both gone.
  deliberate = true;
}
