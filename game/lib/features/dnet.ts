...
import type { DarknetAgentDigest, DarknetKnownHost, DarknetProfit } from "../../../shared/telemetry/topics/dnet.ts";
import {
  CONTROLLER_CALLS,
  KIND_CALLS,
  priceCalls,
...
import type { DnetAgentLaunch, DnetControllerLaunch, DnetProberLaunch } from "../../dnet/launch.ts";
import { emptyDnetProfit, hasDnetProfit, mergeDnetProfit } from "../../dnet/profit.ts";
 ...
 * CONTROLLER onto `darkweb`, drains what the controller has learned into the durable
 * fold it owns, installs backdoors from the terminal (the one darknet action
 * only home can perform), and tells the controller what only home can see. The
 ...
 * survives a controller death because home folds each drained observation into
...
interface DnetHomeState {
  ...
  knowledge: DnetKnowledge | undefined;
  ...
  karmaLoss: number;
  /** Cumulative observed farm returns for this install. */
  ...
  /** The maze as the controller last drew it — the discovered map and whoever is
   * walking it.
   *
   ...
   * telemetry; its only replay is the in-realm order to the controller. What the
   * panel gets is the boolean `credentialKnown` per host.
   *
   * Held here rather than only out in the darknet because a controller dies with
...
function loadPersistedDnetState(
  ...
  for (const entry of persisted.vault) {
    ...
    if (home.knowledge && !home.knowledge.hosts.has(entry.hostname)) {
      const at = entry.at ?? Date.now();
      home.knowledge.hosts.set(entry.hostname, {
...
function drainDarknet(generation: string): {
  ...
  for (const rejection of taken.credentialRejections) {
    ...
    const host = home.knowledge?.hosts.get(rejection.hostname);
  ...
  mergeDnetProfit(home.profit, taken.profit);
  if (taken.grammar) home.grammar = taken.grammar;
  // ASSIGNED, and only when the controller had something to say. The maze is a
  // standing picture rather than a since-last-drain delta, so the newest one
  // wins outright — but a controller that has not reached a lab sends nothing,
  ...
  // controller runs, because the two must not disagree about what a storm
  ...
  if (taken.stormFiredAt !== undefined && taken.stormFiredAt > (home.lastStormAt ?? 0)) {
    ...
    if (home.knowledge !== undefined) {
      home.knowledge = stormWipeKnowledge(home.knowledge, { stasisLinked: home.stasisLinked });
    }
  }
  ...
  home.controllerBeatAt = Math.max(home.controllerBeatAt, rendezvous.lastBeatAt);
...
const dnet: FeatureDriver = {
  ...
  // the hot path. This tick DRAINS the controller and PUBLISHES the map, pure
  ...
  // controller signalling home on drain), not a hotter poll.
  everyMs: 5_000,
  requires: "dnet",
  async tick(ctx: DriverContext) {
    ...
    const folded = foldKnowledgeReports(knowledge, reported, now, expiry);
    ...
    // channel that ever populates `unknownModels`: the controller detects the
    // case, but only home accumulates it across controller lifetimes.
    for (const { hostname, outcome } of reportedAttempts) {
      foldAttempts(home.knowledge.hosts.get(hostname), [outcome]);
    ...
    for (const { hostname, outcome } of reportedLogDrains) {
      foldLogDrain(home.knowledge.hosts.get(hostname), outcome);
    }
    ...
    // rides every `order()` back out to the controller, so a forgotten one never
    // stops being replayed.
    for (const [hostname, entry] of [...home.vault]) {
      const host = home.knowledge.hosts.get(hostname);
      if (!host
    ...
    const cover = knowledgeCoverage(home.knowledge, now, expiry);
    ...
    const deepest = [...home.knowledge.hosts.values()].reduce((found, host) => {
    ...
    for (const host of home.knowledge.hosts.values()) {
    ...
    set(ctx.state, "dnet", {
      ...
      // snapshot's, because home is the copy that survives a controller death.
      ...(home.storm
        ? {
          storm: {
            ...home.storm,
      ...
      ...(hasDnetProfit(home.profit) ? { profit: home.profit } : {}),
    ...
    const decision = stepDarknet({
      ...
      servers: [...home.knowledge.hosts.values()].map((host) => {
    ...
    if (!retiringBuild && (!controllerAlive || !proberAlive || !residentAlive) && now >= home.seedNextAt) {
      ...
      const wantResident = !residentAlive;
      ...
      const seeded = await featureDodgeOn(ctx, "dnet", "action:seed", DNET_SEED_METHODS, "home", async (stubNs: NS) => {
        ...
        if (wantProber && activeController === undefined) {
          return { controller, prober: 0, resident: wantResident ? 0 : -1, pending: true, reason: "controller has not registered yet" };
        }
            ...
            : claim.launch
              ? await handoffLaunch<DnetProberLaunch>(
                ...
                (launchId) => stubNs["exec"](
                  proberFile,
                  "darkweb",
                  temporaryRunOptions({ threads: 1, ramOverride: proberReserveGb(stubNs) }),
                  launchId,
                ),
              )
              : -1;
        ...
        if (!refreshed) {
          return { controller, prober, resident: wantResident ? 0 : -1, reason: "no room on darkweb for its prober" };
        }
        const resident = wantResident
      ...
      const seedPending = seeded.ok && "pending" in seeded.value && seeded.value.pending === true;
      if (seedPending) {
        home.seedNextAt = now;
      } else {
        home.seedAttempts++;
        if (seeded.ok && seeded.value.controller !== 0 && seeded.value.prober !== 0 && seeded.value.resident !== 0) {
          record(
            "seed",
            true,
            `controller ${seeded.value.controller === -1 ? "kept" : `pid ${seeded.value.controller}`}, prober ${seeded.value.prober === -1 ? "kept" : `pid ${seeded.value.prober}`}, resident ${seeded.value.resident === -1 ? "kept" : `pid ${seeded.value.resident}`}`,
          );
...
function dnetSeedWanted(): boolean {
  ...
  const now = Date.now();
  // Either the controller is gone, or darkweb is missing one member of its
  // permanent prober + resident pair.
  if (now - home.controllerBeatAt >= DNET_PROCESS_STALE_MS) return true;
  const resident = dnetRendezvous()?.hosts.get("darkweb");
  return (resident?.prober?.pid ?? 0) <= 0
    || resident?.agent === undefined
    || now - residentLastLife(resident) >= DNET_PROCESS_STALE_MS;
}
