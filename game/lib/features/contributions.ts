...
export class ContributionCache {
  ...
  readonly claimsByFeature: Partial<Record<FeatureId, Claim[]>> = {};
  ...
  replaceClaims(id: FeatureId, claims: Claim[]): Claim[] {
  ...
  claims(transient: readonly Claim[] = []): Claim[] {
    return [...Object.values(this.claimsByFeature).flat(), ...transient];
  }
}

export function isStandingClaim(claim: Claim): boolean {
  return claim.resource === "time" || (claim.resource === "money" && claim.mode === "reserve");
}
