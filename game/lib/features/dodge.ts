...
import { DodgeExecError, priceCalls, startDodge } from '../dodge.ts';
...
export async function featureDodge<T>(
  ...
  let released = false;
  try {
    const started = await startDodge(ctx.ns, body, budgetGb, { host: lease.host });
    lease.release();
    released = true;
    return { ok: true, value: await started.result };
  } catch (error) {
...
export async function featureDodgeOn<T>(
  ...
  const budgetGb = priceCalls(ctx.ns, methods);
  const priority = ctx.grants.ramClaims.get(claimId)?.priority ?? PRIORITY['probe:detail'];
  const lease = ctx.acquireDodge(budgetGb, { by, id: claimId, priority, pinHost });
  if (lease.status === 'queued') {
