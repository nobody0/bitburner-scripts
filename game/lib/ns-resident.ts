...
import { captureExecLaunch } from "./launch-shared.ts";
 ...
 *
 * Keep this file free of value imports beyond `captureExecLaunch` and of any
 * billable ns reference, so its static base stays 1.6 GB and the whole
 * allocation belongs to the proxy's budget. */
export async function main(ns: NS): Promise<void> {
  ns.disableLog("ALL");

  const entity = captureExecLaunch<ProxyLaunch, NS>(ns, "ns-proxy");
  // Missing rendezvous happens after a game restart; die quietly.
  if (!entity) return;
  const launch = entity.descriptor;

  // Generic capture installed the lifetime checkout before publication.
  entity.ready.resolve(ns);
